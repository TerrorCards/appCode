...
import { ScrollView, StyleSheet, FlatList, View, Image, Text, Dimensions, Alert, ImageBackground,
	Picker, TouchableOpacity, TouchableWithoutFeedback, TouchableHighlight, Platform, TextInput } from 'react-native';
...
    this.requestSets({"task":"sets", "filter": "", "userId":this.props.userId});
    this.requestCards({"task":"tradeSetup", "filter": {
		"year": this.state.initYear, 
		"category": "All",
		"receiver": this.props.partner,
		"needs": "N"
	}, "userId":this.props.userId, "owner":"You", "action":"initial", "progress":this.state.progress}).then(() =>{
		this.requestCards({"task":"tradeSetup", "filter": {
			"year": this.state.initYear, 
			"category": "All",
			"receiver": this.props.userId,
			"needs": "N"
		}, "userId":this.props.partner, "owner":"Other", "action":"initial", "progress":this.state.progress}).then(() => {
		...
		if(this.state.progress == 1 || this.state.progress == 2) {
			return(<View>			
			  <View  style={{flex:1, flexDirection:"row", height:40, backgroundColor:"rgba(255,255,255,0.7)"}}>
				...
				<View style={{flex:1, flexDirection:"row", height:40, top:40, backgroundColor:"rgba(255,255,255,0.7)"}}>
				<Picker
				...
				return <Picker.Item label={sets.setName} value={sets.setName} key={sets.setName}/>
			  	...
			  	<View style={{height:100, top:80, backgroundColor:"rgba(255,255,255,0.7)"}}>	
					<ScrollView horizontal={true} style={{height:100}}>{TempList()}</ScrollView>
				</View>
				<View style={{top:85, height:Dimensions.get('window')-150}}>	
				<GridView
					itemDimension={100}
			...
			return(
				...
				<View style={{width: Dimensions.get('window').width - 50}}>
					...
					<View style={{paddingTop:50}}>
						<TextInput
								style={{height: 75, width: Dimensions.get('window').width, borderColor: 'gray', borderWidth: 1}}
		...
		if(currentStep === 2) {
			if(this.state.tradeListPool.length > 0) {				
				...
				this.setState({yourTradeList: this.state.tradeListPool},() =>{
					...
					this.setState({showNext: true, showBack:true, progress: currentStep, progressText:message,
			...
			this.setState({showNext: true, showBack:false, progress: currentStep, progressText:message,
...
decreaseProgress =() => {
	...
	if(currentStep <= 1) {	
		...
		this.setState({otherTradeList: this.state.tradeListPool}, () => {
			...
			this.setState({showBack: false, showNext: true, progress:1, progressText:message,
		...
		message = "I want from " + this.props.partner;
		this.setState({showBack: true, showNext: true, progress: currentStep, progressText:message,
...
updateCardRequest =() => {
	...
	}, "userId":this.props.userId, "owner":"You", "action":"update", "progress":this.state.progress}).then(() =>{
		this.requestCards({"task":"tradeSetup", "filter": {
			"year": this.state.initYear, 
			"category": this.state.initSet,
			"receiver": this.props.userId,
			"needs": this.state.initNeedsOption
		}, "userId":this.props.partner, "owner":"Other", "action":"update", "progress":this.state.progress}).then(() => {
