...
function pullTradeCards(pUserId,pOther) {
	...
	setTimeout(function() {
		...
		callServer('tradeSetup',{receiver:pOther, needs:needsData, category:categoryData, year:yearData},pUserId,loadUserTradeCards);
		callServer('tradeSetup',{receiver:pUserId, needs:needsData, category:categoryData, year:yearData},pOther,loadUserTradeCards);		
	}, 1000);		
}
    ...
    //TODO loop though exisitng list and set the proper count if card list is refreshed but still has cards that exist in the tradelist
      ...
      cardInfo.index = index;
	      ...
	      statHolder = statHolder + '<div>';
	      	...
	      	item = item + '<img src="' + cardInfo.Image + '" id="'+cardInfo.ID+'_image" alt="'+cardInfo.ID+'" class="tradeItemButton" />';
	      	item = item + '</div></li>';
...
function AddTradeList(pItem, pOwner, pID, pArray, pImage) {
		var result = $.grep(pArray, function(e){ return e.ID == pItem.ID; });
		if(result.length > 0) {			
			$.each(pArray, function( index, value ) {
			...
			pItem.RuntimeCount = 1; //force the count in trade list to 1 to start tracking
			var copiedObject = jQuery.extend({}, pItem);
			pArray.push(copiedObject);
			pItem.RuntimeCount = parseInt(pItem.Count) - 1; //set it back so global list has correct runtime count
			if (pItem.RuntimeCount <= 0) {
				$("#"+pImage.id).removeClass( "tradeItemButton" ).addClass( "tradeItemRemove" );
			}			
		}
...
function RemoveFromTradeList(pItem, pClick, pOwner, pIndex, pID, pImage, pArray) {
	var value;
		var popIndx = -1;
		var popId = -1;
		var result = $.grep(pArray, function(e){ return e.ID == pClick.ID; });
		if(result.length > 0) {			
			$.each(pArray, function( index, value ) {
