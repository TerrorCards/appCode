...
function loadBattlePlayers() {
	...
	callServer("battlePlayerList", {rating:gUser.Rating, search:""}, gUser.ID, populateBattlePlayers);	 	
}
...
function populateBattlePlayers(pParams) {
	if(pParams.length > 0) {
		console.log(pParams);
		$('#tblBattlePlayerList').empty();
		$.each(pParams, function (index, value) {
			$('#tblBattlePlayerList').append('<div style="padding:10px" onclick=propAcceptBattle({target:"'+value.Player+'"})><img src="' + value.Img + '" width="50">' + value.Player + '</div>');
		});
	}
...
function propAcceptBattle(pParam) {
	myApp.confirm('This will cost 1 battle, proceed?', 'Terror Cards', function () {
  ...
  if(pArray.length > 0) {
      ...
      $.each(pArray, function (index, value) { 		       	       
	  	if(value.Player == gUser.ID) {
	  		console.log(value);
	  		$("#tdBattleMoves").text(value.MovesLeft);
...
function searchHGPlayer() {
	callServer("battlePlayerList", {rating:gUser.Rating, search:$("#HGplayerSearch").val()}, gUser.ID, populateBattlePlayers);	
}
