...
function loadGalleryImages(pArray) {
  if(pArray.length > 0) {
	...
	$("#txtSearch").keyup(function() {  	
  	...
  	/*
  	var howMany = pArray.length / 50;
  	if(howMany > 1) {
  		var arrayPiece = chunkArray(pArray,50);	
  	} else {
		var arrayPiece = pArray;  		
  	}
   	*/
  	...
  	var counter = 0;
  	var blocks = [];
  	$.each(arrayPiece, function( index, piece ) {
  		if(Array.isArray(piece)) {
  			$.each(piece, function( index2, value ) {
		  		pieceLoadGallery(index2, value, cardInfo, statHolder, currCard, currSet, currCount, counter);  				
  			}); 
  			//lzl.update(); 	
  		} else {
    		pieceLoadGallery(index, piece, cardInfo, statHolder, currCard, currSet, currCount, counter);
    		//lzl.update();
    	} 			
  	});
  	//lzl.update();
...
function pieceLoadGallery(index, value, cardInfo, statHolder, currCard, currSet, currCount, counter) {
      cardInfo = value;
	  ...
	  statHolder = statHolder + '<div>';
		...
		if(cardInfo.Count !== null) {
			$imgContent = '<li>'+statHolder+'<img class="lazy" src="images/waitSmall.png" data-src="' + thumbImg + '" id="'+cardInfo.Name+cardInfo.SetName+'_gallery" alt="image" /></li>';
		} else {
			$imgContent = '<li>'+statHolder+'<img class="lazy" src="images/waitSmall.png" data-src="' + thumbImg + '" id="'+cardInfo.Name+cardInfo.SetName+'_gallery" alt="image" style="opacity: 0.3" /></li>';	
		}
